...
const App = () => {
    ...
    const handleAutoBackup = useCallback(async () => {
        ...
        try {
            ...
            setConfirmation({ isOpen: true, title: 'Automated Backup', message: 'A scheduled system backup has been successfully saved to the cloud database.', type: 'success' });
        } catch (e) {
            ...
            case 'estimates': 
                return (
                    <EstimatesView 
                        ...
                        onOpenServicePackageModal={(pkg) => setServicePackageFormModal({ isOpen: true, servicePackage: pkg })} 
                    />
                );
            case 'invoices': 
                return (
                    <InvoicesView 
                        ...
                        vehicles={vehicles}
                        taxRates={taxRates}
                        onViewInvoice={(inv) => setViewInvoiceModal({isOpen: true, invoice: inv})} 
                        onEditInvoice={(inv) => setInvoiceFormModal({isOpen: true, invoice: inv})} 
                        onOpenExportModal={(type, items) => setExportModal({isOpen: true, type, items})} 
                        onCreateAdhocInvoice={() => setInvoiceFormModal({isOpen: true, invoice: { createdByUserId: currentUser.id } as any})} 
                    />
                );
            case 'purchaseOrders': 
                return (
                    <PurchaseOrdersView 
                        ...
                        suppliers={suppliers}
                        onOpenPurchaseOrderModal={(po) => setPoModal({isOpen: true, po})} 
                        onViewPurchaseOrder={(po) => setViewPoModal({isOpen: true, po})} 
                        onDeletePurchaseOrder={(id) => handleDeleteItem(setPurchaseOrders, id)} 
                        onExport={() => {}} 
                        onOpenBatchAddModal={() => setBatchPoModalOpen(true)} 
                    />
                );
            case 'sales': return <SalesView entity={businessEntities.find(e => e.id === selectedEntityId)!} onManageSaleVehicle={(sv) => setSorContractModal({isOpen: false, saleVehicle: null}) } onAddSaleVehicle={() => {}} onGenerateReport={() => setSalesReportModal(true)} onAddProspect={() => setProspectModal({isOpen: true, prospect: null})} onEditProspect={(p) => setProspectModal({isOpen: true, prospect: p})} onViewCustomer={() => {}} />;
