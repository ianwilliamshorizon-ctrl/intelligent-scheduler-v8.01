...
import { X, Save, HardDriveDownload } from 'lucide-react';
...
const FormModal: React.FC<FormModalProps> = ({ isOpen, onClose, title, children, onSave, saveText = "Save", saveIcon: SaveIcon = Save, maxWidth = "max-w-3xl", onForceSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 z-[70] flex justify-center items-center p-4">
      <div className={`bg-white rounded-xl shadow-2xl w-full ${maxWidth} flex flex-col max-h-[90vh] animate-fade-in-up overflow-y-auto`}>
        ...
        {(onSave || onForceSave) && (
          <div className="flex justify-end p-4 border-t bg-gray-50 flex-shrink-0 sticky bottom-0 z-10 items-center">
            {onForceSave && (
                <button onClick={onForceSave} className="flex items-center py-2 px-4 bg-amber-500 text-white font-semibold rounded-lg shadow-md hover:bg-amber-600 transition mr-auto">
                  <HardDriveDownload size={16} className="mr-2"/> Force Save
                </button>
            )}
