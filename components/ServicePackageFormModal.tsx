...
const ServicePackageFormModal = ({ isOpen, onClose, onSave, servicePackage, taxRates, entityId, businessEntities, parts }: { isOpen: boolean, onClose: () => void, onSave: (pkg: ServicePackage) => void, servicePackage: Partial<ServicePackage> | null, taxRates: TaxRate[], entityId: string, businessEntities: BusinessEntity[], parts: Part[] }) => {
    ...
    const executeSave = () => {
        ...
        if(!finalName || formData.totalPrice === undefined) { 
            showToast('Please provide a name and total price', 'error');
            return;
        }

        // Spread the formData, but don't force a UUID if one doesn't exist
        // This allows the parent/Firestore to decide the ID
        onSave({ 
            ...formData, 
            name: finalName 
        } as ServicePackage);

        showToast('Service package save request sent!', 'success');
        onClose();
    }
    ...
    return (
        <FormModal isOpen={isOpen} onClose={onClose} onSave={executeSave} title={servicePackage?.id ? "Edit Service Package" : "Add Service Package"} maxWidth="max-w-5xl">
