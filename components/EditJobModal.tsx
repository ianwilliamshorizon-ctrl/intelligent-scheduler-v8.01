            ...
            if (newPartItems.length > 0) {
                ...
                for (const supplierId in partsBySupplier) {
                    ...
                    if (draftPO) {
                        ...
                        // Replace existing update in updatedPOs or add new
                        ...
                        // Generate a temporary ID that doesn't conflict with existing ones in this transaction
                        const existingPoIds = [...purchaseOrders, ...newPOs].map(p => p.id);
                        // We need a way to generate sequential ID here, but generatePurchaseOrderId needs the full array.
                        // We simulate the array with newPOs included.
